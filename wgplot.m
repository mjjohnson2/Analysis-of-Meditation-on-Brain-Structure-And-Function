...
if eWeighted
  ...
  for kk=1:neColor
    ...
    y=[coord(ii(p),2),coord(jj(p),2),repmat(nan,nSegment,1)]';
            z=[coord(ii(p),3),coord(jj(p),3),repmat(nan,nSegment,1)]';%%

    hE=[hE,plot3(x(:),y(:),z(:),'color',edgeMap(kk,:),plotParm{:})];%%
    % Draw legend bar
  ...
  caxis([minEWt maxEWt])
    ...
    y=[coord(ii,2),coord(jj,2),nan(nSegment,1)]';
        z=[coord(ii,3),coord(jj,3),nan(nSegment,1)]';%%

    hE=plot3(x(:),y(:),z(:),plotParm{:});%%
end  % if eWeighted
...
if vWeighted
  ...
  if exist('vrtxCol','var')
    colormap(vrtxMap);
    hV=scatter3(coord(qq,1),coord(qq,2),coord(qq,3),vWt,vrtxCol,'filled');
    colorbar;
  else
    hV=scatter3(coord(qq,1),coord(qq,2),coord(qq,3),vWt,'filled','MarkerFaceColor',[1,0.5,0.2]);
  end
  
else
  %hV=plot(coord(:,1),coord(:,2),plotParm{:},'LineStyle','none');
  hV=plot3(coord(qq,1),coord(qq,2),coord(qq,3),plotParm{:},'LineStyle','none');
end
